...
class S2Member_Tools_Pages {
	 ...
	 * @return  mixed
	 */
	static public function add_page_list_columns_head ( $defaults ) {
	 ...
	 * @return  void
	 */
	static public function add_page_list_columns_data( $column_name, $page_id ) {

		if ( 's2m_memberlevel' == $column_name ) {
			$level = 0;
			$s2moptions = S2Member_Tools::get_s2m_options();
			while (true) {
				if ( $s2moptions[ 'level' . $level . '_pages' ]  ) {
					if ( in_array( $page_id, explode(',', $s2moptions[ 'level' . $level . '_pages' ] ) ) ) {
